...
//Malcolm: exec "node simpleMicStream.js 8088" on server
//Malcolm: visit "http://server-ip:8088" on browser
...
http.createServer(function (req, res) {
  ...
  if (req.url === '/') {
    ...
    if (ps === null) {
      ...
      var ps1=spawn('arecord', ['-f', 'cd', '-t', 'raw'])
      ps = spawn('lame', ['-r', '-', '-'])
      ps1.stdout.pipe(ps.stdin);
